...
/**
 * A wrapper class that allows block ciphers to be used to process data in a piecemeal fashion. The BufferedBlockCipher outputs a block only when the
...
public class BufferedBlockCipher {
     ...
     * @param padded
     *            true if the buffer should add, or remove, pad bytes, false otherwise.
     */
    public BufferedBlockCipher(BlockCipher cipher) {
        ...
        if (pgpCFB) {
            ...
            partialBlockOkay =
                    (idx > 0 && (name.startsWith("CFB", idx) || name.startsWith("OFB", idx) || name.startsWith("OpenPGP", idx)
                            || name.startsWith("SIC", idx) || name.startsWith("GCTR", idx)));
        }
    }
     ...
     * @return the number of output bytes copied to out.
     * @exception DataLengthException
     *                if there is insufficient space in out for the output, or the input is not block size aligned and should be.
     * @exception IllegalStateException
