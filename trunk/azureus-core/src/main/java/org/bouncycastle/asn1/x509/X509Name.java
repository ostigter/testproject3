...
public class X509Name extends ASN1Encodable {
     ...
     * <p>
     * it's is assumed the table contains OID/String pairs, and the contents of the table are copied into an internal table as part of the
     * construction process.
     * <p>
     * <b>Note:</b> if the name you are trying to generate should be following a specific ordering, you should use the constructor with the ordering
     * specified below.
     * 
     ...
     * <p>
     * it's is assumed the table contains OID/String pairs, and the contents of the table are copied into an internal table as part of the
     * construction process. The ordering vector should contain the OIDs in the order they are meant to be encoded or printed in toString.
     */
    ...
    /**
     * Takes an X509 dir name as a string of the format "C=AU, ST=Victoria", or some such, converting it into an ordered set of name attributes with
     * each string value being converted to its associated ASN.1 type using the passed in converter.
     */
    ...
    /**
     * Takes an X509 dir name as a string of the format "C=AU, ST=Victoria", or some such, converting it into an ordered set of name attributes. If
     * reverse is true, create the encoded version of the sequence starting from the last element in the string.
     */
    ...
    /**
     * Takes an X509 dir name as a string of the format "C=AU, ST=Victoria", or some such, converting it into an ordered set of name attributes with
     * each string value being converted to its associated ASN.1 type using the passed in converter. If reverse is true the ASN.1 sequence
     * representing the DN will be built by starting at the end of the string, rather than the start.
     */
    ...
    /**
     * Takes an X509 dir name as a string of the format "C=AU, ST=Victoria", or some such, converting it into an ordered set of name attributes.
     * lookUp should provide a table of lookups, indexed by lowercase only strings and yielding a DERObjectIdentifier, other than that OID. and
     * numeric oids will be processed automatically. <br>
     * If reverse is true, create the encoded version of the sequence starting from the last element in the string.
     * 
     * @param reverse
     *            true if we should start scanning from the end (RFC 2553).
     * @param lookUp
     *            table of names and their oids.
     * @param dirName
     ...
     * Takes an X509 dir name as a string of the format "C=AU, ST=Victoria", or some such, converting it into an ordered set of name attributes.
     * lookUp should provide a table of lookups, indexed by lowercase only strings and yielding a DERObjectIdentifier, other than that OID. and
     * numeric oids will be processed automatically. The passed in converter is used to convert the string values to the right of each equals sign to
     * their ASN.1 counterparts. <br>
     * 
     * @param reverse
     *            true if we should start scanning from the end, false otherwise.
     * @param lookUp
     *            table of names and oids.
     * @param dirName
     *            the string dirName
     * @param converter
    ...
    private void appendValue(StringBuffer buf, Hashtable oidSymbols, DERObjectIdentifier oid, String value) {
        ...
        while (index != end) {
            if ((buf.charAt(index) == ',') || (buf.charAt(index) == '"') || (buf.charAt(index) == '\\') || (buf.charAt(index) == '+')
                    || (buf.charAt(index) == '<') || (buf.charAt(index) == '>') || (buf.charAt(index) == ';')) {
                buf.insert(index, "\\");
                index++;
                end++;
            }
    ...
    /**
     * convert the structure to a string - if reverse is true the oids and values are listed out starting with the last element in the sequence (ala
     * RFC 2253), otherwise the string will begin with the first element of the structure. If no string definition for the oid is found in oidSymbols
     * the string value of the oid is added. Two standard symbol tables are provided DefaultSymbols, and RFC2253Symbols as part of this class.
     * 
     * @param reverse
     *            if true start at the end of the sequence and work back.
     * @param oidSymbols
