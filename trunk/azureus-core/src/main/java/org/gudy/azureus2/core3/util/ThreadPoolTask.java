...
public abstract class ThreadPoolTask extends AERunnable {
    ...
    /**
     * only invoke this method after the first run of the threadpooltask as it is only meant to join on a task when it has child tasks and thus is
     * running in manual release mode
     */
    ...
    synchronized final void setManualRelease() {
        manualRelease = ThreadPoolTask.RELEASE_MANUAL;
    }

    synchronized final boolean canManualRelease() {
        return (manualRelease == ThreadPoolTask.RELEASE_MANUAL_ALLOWED);
    }

    /**
     * only invoke this method after the first run of the threadpooltask as it is only meant to update the state of a task when it has child tasks and
     * thus is running in manual release mode
     */
