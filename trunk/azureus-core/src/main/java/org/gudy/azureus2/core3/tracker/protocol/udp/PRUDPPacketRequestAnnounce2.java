...
public class PRUDPPacketRequestAnnounce2 extends PRUDPPacketRequest {
     ...
     * int m_ipa; INT KEY // V2 <XTF> int m_num_want; <XTF> short m_port;
     */
    ...
    public void setDetails(byte[] _hash, byte[] _peer_id, long _downloaded, int _event, int _ip_address, int _key, int _num_want, long _left,
            short _port, long _uploaded) {
        hash = _hash;
        peer_id = _peer_id;
    ...
    public String getString() {
        return (super.getString()).concat("[").concat(
                "hash=" + ByteFormatter.nicePrint(hash, true) + "peer=" + ByteFormatter.nicePrint(peer_id, true) + "dl=" + downloaded + "ev="
                        + event + "ip=" + ip_address + "key=" + key + "nw=" + num_want + "left=" + left + "port=" + port + "ul=" + uploaded + "]");
    }
}
