...
public class XMLElement {
    ...
    /**
     * This is what the XMLElement holds. It is either: a) A single string (text_content); or b) A collection of XMLElements.
     * 
     * Both are null at construction time - only one can be set.
     */
    ...
    private static class ContentComparator implements java.util.Comparator<XMLElement> {
        public int compare(XMLElement xe1, XMLElement xe2) {
            ...
            /**
             * This is necessary - we don't expect to deal with two elements which are fundamentally equal, but we may be asked to compare the same
             * actual object against itself when we first populate the collection which uses this comparator.
             * 
             * Ideally, we don't want to allow semantically equivalent objects in the collection, because it's not clear what we're expected to do
             * when we're being asked to order elements. We don't expect, in the general use case, to have a caller trying to add the same element
             * twice without providing a differing index attribute (what are they trying to achieve - two copies at the same time or for it to be
             * silently dropped?)
             * 
             * The behaviour where we compare the same object against itself was introduced here in Java 7.
            ...
            /**
             * This is the situation we want to avoid (the one I try to describe much earlier in the method) - the two elements aren't the same
             * instance and don't differ enough or have enough information to describe a natural ordering.
             */
            //
            throw new IllegalArgumentException(
                    "Shouldn't be using sorting for contents if you have tags with same name and no index attribute! (tag: " + xe1.getTag() + ")");
        }
    }
