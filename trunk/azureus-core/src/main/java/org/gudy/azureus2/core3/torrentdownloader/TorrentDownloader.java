...
public interface TorrentDownloader {
    ...
    /**
     * Changes the path and filename to download to. You can give <code>null</code> for either to leave it as is. (These are initialized to either the
     * path/filename given via <code>TorrentDownloaderFactory.download(Managed)</code> or to the default torrent save directory (path) and the
     * filename the server proposes (file). This function does nothing after the download has been started.
     * 
     * @param path
