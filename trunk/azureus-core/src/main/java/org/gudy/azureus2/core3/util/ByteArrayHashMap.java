...
public class ByteArrayHashMap<T> {
    ...
    /*
     * public static void main( String[] args ) { ByteArrayHashMap map = new ByteArrayHashMap(); byte[][] keys = new byte[1024][]; Random random = new
     * Random(); for (int i=0;i<keys.length;i++){ byte[] key = new byte[random.nextInt(32)]; random.nextBytes(key); keys[i] = key; if ( i < 512 ){
     * map.put( key, new Object()); } } for (int i=0;i<keys.length;i++){ System.out.println( i + " - " + map.get( keys[i] )); } }
     */
}
