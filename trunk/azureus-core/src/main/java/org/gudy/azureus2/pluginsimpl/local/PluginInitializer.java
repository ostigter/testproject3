...
public class PluginInitializer implements GlobalManagerListener, AEDiagnosticsEvidenceGenerator {
    ...
    private String[][] builtin_plugins = {
            ...
            { PluginManagerDefaults.PID_REMOVE_RULES, "com.aelitis.azureus.plugins.removerules.DownloadRemoveRulesPlugin", "azbpremovalrules", "",
                    "true", "false" },
            { PluginManagerDefaults.PID_SHARE_HOSTER, "com.aelitis.azureus.plugins.sharing.hoster.ShareHosterPlugin", "azbpsharehoster",
                    "ShareHoster", "true", "false" },
            { PluginManagerDefaults.PID_PLUGIN_UPDATE_CHECKER, "org.gudy.azureus2.pluginsimpl.update.PluginUpdatePlugin", "azbppluginupdate",
                    "PluginUpdate", "true", "true" },
            { PluginManagerDefaults.PID_UPNP, "com.aelitis.azureus.plugins.upnp.UPnPPlugin", "azbpupnp", "UPnP", "true", "false" },
            { PluginManagerDefaults.PID_DHT, "com.aelitis.azureus.plugins.dht.DHTPlugin", "azbpdht", "DHT", "true", "false" },
            { PluginManagerDefaults.PID_DHT_TRACKER, "com.aelitis.azureus.plugins.tracker.dht.DHTTrackerPlugin", "azbpdhdtracker", "DHT Tracker",
                    "true", "false" },
            ...
            // "azjpc",
            // "false" },
            { PluginManagerDefaults.PID_EXTERNAL_SEED, "com.aelitis.azureus.plugins.extseed.ExternalSeedPlugin", "azextseed", "azextseed", "true",
                    "false" },
            { PluginManagerDefaults.PID_LOCAL_TRACKER, "com.aelitis.azureus.plugins.tracker.local.LocalTrackerPlugin", "azlocaltracker",
                    "azlocaltracker", "true", "false" },
            { PluginManagerDefaults.PID_NET_STATUS, "com.aelitis.azureus.plugins.net.netstatus.NetStatusPlugin", "aznetstat", "aznetstat", "true",
                    "false" },
            { PluginManagerDefaults.PID_BUDDY, "com.aelitis.azureus.plugins.net.buddy.BuddyPlugin", "azbuddy", "azbuddy", "true", "false" },
    ...
    public List loadPlugins(AzureusCore core, boolean bSkipAlreadyLoaded, boolean load_external_plugins, boolean loading_for_startup,
        ...
        if (load_external_plugins) {
            pluginLoaded.addAll(loadPluginsFromDir(user_dir, 0, user_plugins + app_plugins, bSkipAlreadyLoaded, loading_for_startup,
                    initialise_plugins));

            if (!user_dir.equals(app_dir)) {

                pluginLoaded.addAll(loadPluginsFromDir(app_dir, user_plugins, user_plugins + app_plugins, bSkipAlreadyLoaded, loading_for_startup,
                        initialise_plugins));
            }
        } else {
            if (Logger.isEnabled()) {
        ...
        for (int i = 0; i < builtin_plugins.length; i++) {
            ...
            if (def.isDefaultPluginEnabled(builtin_plugins[i][0])) {
                ...
                if (core_operation != null) {
                    core_operation.reportCurrentTask(MessageText.getString("splash.plugin") + builtin_plugins[i][0]);
                }
    ...
    private List loadPluginsFromDir(File pluginDirectory, int plugin_offset, int plugin_total, boolean bSkipAlreadyLoaded,
            boolean loading_for_startup, boolean initialise) {
    ...
    private List loadPluginFromDir(File directory, boolean bSkipAlreadyLoaded, boolean loading_for_startup, boolean initialise) // initialise setting
        ...
        if (!looks_like_plugin) {
            ...
            if (Logger.isEnabled())
                Logger.log(new LogEvent(LOGID, LogEvent.LT_WARNING, "Plugin directory '" + directory + "' has no plugin.properties "
            ...
            while (true) {
                ...
                if (existing_pi != null) {
                    ...
                    if (this_parent.equals(FileUtil.getApplicationFile("plugins")) && existing_parent != null
                        ...
                        Logger.log(new LogAlert(LogAlert.UNREPEATABLE, LogAlert.AT_WARNING, "Error loading '" + plugin_name_string
                                + "', plugin class '" + plugin_class + "' is already loaded"));
                    }
                    ...
                    public void run() {
                        try {
                            ...
                            if (l.size() > 0) {
                                ...
                                if (core_operation != null) {
                                    core_operation.reportCurrentTask(MessageText.getString("splash.plugin.init") + " "
                    ...
                    public void run() {
                        if (def.isDefaultPluginEnabled(builtin_plugins[idx][0])) {
                            ...
                            try {
                                ...
                                if (Logger.isEnabled())
                                    Logger.log(new LogEvent(LOGID, LogEvent.LT_WARNING, "Initialization of built in plugin '"
                                            + builtin_plugins[idx][2] + "' complete"));
                            } catch (Throwable e) {
                                try {
                                ...
                                if (builtin_plugins[idx][4].equalsIgnoreCase("true")) {
                                    Debug.printStackTrace(e);
                                    Logger.log(new LogAlert(LogAlert.UNREPEATABLE, "Initialization of built in plugin '" + builtin_plugins[idx][2]
                                            + "' fails", e));
                                }
                            }
                        } else {
    ...
    protected void initializePluginFromClass(final Class plugin_class, final String plugin_id, String plugin_config_key, boolean force_enabled,
            boolean loading_for_startup, boolean initialise)
    ...
    throws PluginException {
        ...
        if (plugin_class != FailedPlugin.class && getPluginFromClass(plugin_class) != null) {
            ...
            Logger.log(new LogAlert(LogAlert.UNREPEATABLE, LogAlert.AT_WARNING, "Error loading '" + plugin_id + "', plugin class '"
    ...
    protected void fireEventSupport(final int type, final Object value) {
        async_dispatcher.dispatch(new AERunnable() {
            public void runSupport() {
                ...
                if (type == PluginEvent.PEV_CONFIGURATION_WIZARD_STARTS || type == PluginEvent.PEV_CONFIGURATION_WIZARD_COMPLETES
                        || type == PluginEvent.PEV_INITIAL_SHARING_COMPLETE || type == PluginEvent.PEV_INITIALISATION_UI_COMPLETES
