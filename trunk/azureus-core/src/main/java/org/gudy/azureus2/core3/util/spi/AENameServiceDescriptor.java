    ...
    static {
        ...
        try {
            ...
            if (default_ns != null) {
                ...
                new_ns =
                        (NameService) Proxy.newProxyInstance(NameService.class.getClassLoader(), new Class[] { NameService.class },
        ...
        /*
         * It almost works by delegating the the DNSNameService directly apart from InetAddress.getLocalHost() - rather than returning something
         * sensible it fails with unknown host. However, if we directly grab the InetAddressImpl and use this (which has already been set up to use
         * the default name server) things work better :( Hacked to support both at the moment...
         */
