...
/**
 * this is a telnet UI that starts up a server socket that listens for new connections on a (configurable) port. when an incoming connection is
 * recieved, we check the host against our list of allowed hosts and if this host is permitted, we start a new command line interface for that
 * connection.
 * 
 * @author fatal
 */
public class UI extends org.gudy.azureus2.ui.common.UITemplateHeadless implements IUserInterface {
    ...
    public void startUI() {
        if (!isStarted()) {
            try {
                ...
                Thread thread =
                        new Thread(new SocketServer(this, telnetPort, allowedHosts, userManager, maxLoginAttempts), "Telnet Socket Server Thread");
    ...
    /**
     * creates a new console input using the specified input/output streams. we create the new input in non-controlling mode because we don't want the
     * 'quit' command to shut down the whole interface - simply this clients connection.
     * 
     * @param consoleName
     * @param inputStream
     * @param outputStream
    ...
    public void createNewConsoleInput(String consoleName, InputStream inputStream, PrintStream outputStream, UserProfile profile) {
        ...
        if (userManager != null) {
            MultiUserConsoleInput muc =
                    new MultiUserConsoleInput(consoleName, UIConst.getAzureusCore(), new InputStreamReader(inputStream), outputStream,
                            Boolean.FALSE, profile);
            muc.registerCommand(new UserCommand(userManager));
            console = muc;
        } else {
            console =
                    new ConsoleInput(consoleName, UIConst.getAzureusCore(), new InputStreamReader(inputStream), outputStream, Boolean.FALSE,
                            profile);
