...
public class PlatformDevicesMessenger {
    ...
    public static void qosTurnOn(boolean withITunes, boolean bugFix) {
        ...
        PlatformMessage message =
                new PlatformMessage("AZMSG", LISTENER_ID, OP_QOS_TURN_ON, new Object[] { "itunes", Boolean.valueOf(withITunes), "os-name",
                        Constants.OSName + (bugFix ? ":BF" : "") }, 5000);
    ...
    private static void _qosFoundDevice(Device device) {
        ...
        PlatformMessage message = new PlatformMessage("AZMSG", LISTENER_ID, OP_QOS_FOUND_DEVICE, map, 5000);
        message.setSendAZID(false);
        PlatformMessenger.queueMessage(message, null);
    }
    ...
    private static String getDeviceName(Device device) {
        ...
        /*
         * String name = device.getName(); String classification = device.getClassification(); StringBuffer deviceName = new StringBuffer(); if
         * (device.isGenericUSB()) { deviceName.append("{g}"); } if (device.isHidden()) { deviceName.append("{h}"); } deviceName.append(name); if
         * (!name.equals(classification)) { deviceName.append('/'); deviceName.append(classification); } return deviceName.toString();
         */
    }
    ...
    private static void sendDeviceList(Device[] devices) {
        ...
        PlatformMessage message = new PlatformMessage("AZMSG", LISTENER_ID, OP_REPORT_DEVICES, new Object[] { "renderers", listRenderers }, 500);
        PlatformMessenger.queueMessage(message, null);
    }
}
