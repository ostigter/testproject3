 ...
 * 
 * This class will keep track of the "status" (i.e. seeding, downloading)of the application. It will then re-adjust the MAX limits when it thinks
 * limits are being reached.
 * 
 * Here are the rules it will use.
 * 
 * #1) When seeding. If the upload is AT_LIMIT for a period of time it will allow that to adjust upward. #2) When downloading. If the download is
 * AT_LIMIT for a period of time it will allow that to adjust upward.
 * 
 * #3) When downloading, if a down-tick is detected and the upload is near a limit, it will drop the upload limit to 80% of MAX_UPLOAD.
 * 
...
public class SpeedLimitMonitor implements PSMonitorListener {
    ...
    public void updateSettingsFromCOConfigManager() {
        percentUploadCapacityDownloadMode =
                (float) COConfigurationManager.getIntParameter(SpeedLimitMonitor.USED_UPLOAD_CAPACITY_DOWNLOAD_MODE, 60) / 100.0f;
    ...
    public void updateFromCOConfigManager() {
        ...
        downloadLimitConf =
                SpeedLimitConfidence.parseString(COConfigurationManager.getStringParameter(SpeedLimitMonitor.DOWNLOAD_CONF_LIMIT_SETTING));
    ...
    public void logPMDataEx() {
        ...
        SpeedLimitConfidence tuploadLimitConf =
                SpeedLimitConfidence.parseString(COConfigurationManager.getStringParameter(SpeedLimitMonitor.UPLOAD_CONF_LIMIT_SETTING));
        SpeedLimitConfidence tdownloadLimitConf =
                SpeedLimitConfidence.parseString(COConfigurationManager.getStringParameter(SpeedLimitMonitor.DOWNLOAD_CONF_LIMIT_SETTING));
    ...
    /**
     * If a progressive download is currently active. Then the download limit should not be allowed to go below that limit, regardless of anything
     * else.
     * 
     * @param update
     *            -
    ...
    private SMUpdate checkActiveProgressiveDownloadLimit(SMUpdate update) {
        ...
        if (prgDownLimit * MULTIPLE > update.newDownloadLimit && update.newDownloadLimit != 0) {
            log("Active Progressive download in progress. Overriding limit. curr=" + update.newDownloadLimit + " progDownloadLimit=" + prgDownLimit
                    * MULTIPLE);
    ...
    /**
     * If setting is less then 100kBytes take 1 kByte steps. If setting is less then 500kBytes take 5 kByte steps. if setting is larger take 10 kBytes
     * steps.
     * 
     * @param currLimitMax
    ...
    /**
     * Make a decision about unpinning either the upload or download limit. This is based on the time we are saturating the limit without a down-tick
     * signal.
     */
            ...
            if (uploadAtLimitStartTime + (TIME_AT_LIMIT_BEFORE_UNPINNING * uploadChokePingCount) < currTime) {
                ...
                if (isUploadConfidenceLow()) {
                    ...
                    if (!isUploadConfidenceAbsolute()) {
                        ...
                        isUploadMaxPinned = false;
                        SpeedManagerLogger.trace("unpinning the upload max limit!! #choke-pings=" + uploadChokePingCount + ", pin-counter="
                                + uploadPinCounter);
                    }
                }
            }
    ...
    public synchronized SMUpdate endLimitTesting() {
        ...
        if (transferMode.getMode() == TransferMode.State.DOWNLOAD_LIMIT_SEARCH) {
            ...
            // set that value.
            SpeedManagerLogger.trace("pre-upload-setting=" + preTestUploadCapacity + " up-capacity" + uploadLimitMax + " pre-download-setting="
    ...
    /**
     * After a test is complete determine how condifent the client should be in it based on how different it is from the previous result. If the new
     * result is within 20% of the old result then give it a MED. If it is great then give it a LOW.
     * 
     ...
     * Test for that condition here.
     * 
     * @param currUploadLimit
     *            - reported upload capacity from the adapter
     * @param currDownloadLimit
    ...
    public void setRefLimits(SpeedManagerLimitEstimate estUp, SpeedManagerLimitEstimate estDown) {
        ...
        if (downMax < upMax) {
            SpeedManagerLogger
                    .trace("down max-limit was less then up-max limit. increasing down max-limit. upMax=" + upMax + " downMax=" + downMax);
            downMax = upMax;
        }
    ...
    public void setRefLimits(int uploadMax, int downloadMax) {
        ...
        SpeedManagerLogger.trace("setRefLimits uploadMax=" + uploadMax + " uploadLimitMax=" + uploadLimitMax + ", downloadMax=" + downloadMax
                + " downloadLimitMax=" + downloadLimitMax);
    ...
    public SMUpdate adjustLimitsToSpec(int currUploadLimit, int currDownloadLimit) {
        ...
        SpeedManagerLogger.trace("Adjusting limits due to out of spec: new-up=" + newUploadLimit + " new-down=" + newDownloadLimit + "  reasons: "
                + reason.toString());
     ...
     *            -
     */
    public void betaLogPingMapperEstimates(String name, SpeedManagerLimitEstimate transEst, boolean hadChockPing,
            SpeedManagerLimitEstimate permEst, PingSpaceMapper downMode, PingSpaceMapper seedMode) {
    ...
    public void notifyDownload(SpeedManagerLimitEstimate estimate) {
        ...
        SpeedManagerLogger.trace("notifyDownload downloadLimitMax=" + downloadLimitMax + " conf=" + downloadLimitConf.getString() + " ("
                + downloadLimitConf.asEstimateType() + ")");
        tempLogEstimate(estimate);
