...
public class PlatformSubscriptionsMessenger {
    ...
    public static void updateSubscription(boolean create, String name, byte[] public_key, byte[] private_key, byte[] sid, int version,
            String content)
    ...
    throws PlatformMessengerException {
        ...
        try {
            ...
            /*
             * Signature verify = CryptoECCUtils.getSignature( CryptoECCUtils.rawdataToPubkey( public_key )); verify.update( ( name + pk_str + sid_str
             * + version + content ).getBytes( "UTF-8" )); boolean ok = verify.verify( sig_bytes );
             */
    ...
    public static subscriptionDetails getSubscriptionBySID(byte[] sid)
