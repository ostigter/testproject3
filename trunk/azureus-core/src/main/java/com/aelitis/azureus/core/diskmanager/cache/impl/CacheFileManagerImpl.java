...
public class CacheFileManagerImpl implements CacheFileManager, AEDiagnosticsEvidenceGenerator {
    ...
    protected void initialise(boolean enabled, boolean enable_read, boolean enable_write, long size, long not_smaller_than) {
        ...
        if (Logger.isEnabled())
            Logger.log(new LogEvent(LOGID, "DiskCache: enabled = " + cache_enabled + ", read = " + cache_read_enabled + ", write = "
    ...
    throws CacheFileManagerException {
        ...
        try {
            ...
            } else if ((tf != null && tf.getLength() < cache_files_not_smaller_than) || !cache_enabled
                    || cache_mode == CacheFileOwner.CACHE_MODE_NO_CACHE) {
    ...
    /**
     * allocates space but does NOT add it to the cache list due to synchronization issues. Basically the caller mustn't hold their monitor when
     * calling allocate, as a flush may result in one or more other files being flushed which results in their monitor being taken, and we've got an
     * A->B and B->A classic deadlock situation. However, we must keep the file's cache and our cache in step. It is not acceptable to have an entry
     * inserted into our records but not in the file's as this then screws up the flush algorithm (which assumes that if it finds an entry in our
     * list, a flush of that file is guaranteed to release space). Therefore we add the cache entry in addCacheSpace so that the caller can safely do
     * this while synchronised firstly on its monitor and then we can sync on our. Hence we only ever get A->B monitor grabs which won't deadlock
     * 
     * @param file
     * @param buffer
     * @param file_position
    ...
    throws CacheFileManagerException {
        ...
        if (log && Logger.isEnabled()) {
            ...
            Logger.log(new LogEvent(torrent, LOGID, "DiskCache: cr=" + cache_bytes_read + ",cw=" + cache_bytes_written + ",fr=" + file_bytes_read
    ...
    protected void cacheStatsAndCleaner() {
        SimpleTimer.addPeriodicEvent("CacheFile:stats+cleaner", STATS_UPDATE_FREQUENCY, new TimerEventPerformer() {
    ...
    throws CacheFileManagerException {
        ...
        try {
            ...
            /*
             * if ( entry.getType() == CacheEntry.CT_READ_AHEAD ){ if ( entry.getUsageCount() < 2 ){ System.out.println( "ra: not used" ); }else{
        ...
        for (int i = firstFile; -1 < i && i < files.length; i++) {
            ...
            if (cache_file != null)
                ...
                for (int j = 0; j < results.length; j++)
                    // check if any chunks fall into this non-file
                    if ((absoluteOffsets[j] < fileOffset + length && absoluteOffsets[j] > fileOffset)
                            || (absoluteOffsets[j] + lengths[j] < fileOffset + length && absoluteOffsets[j] + lengths[j] > fileOffset))
        ...
        try {
            ...
            while (it.hasNext()) {
                ...
                if (!files.contains(file)) {
                    ...
                    writer.println("File: " + Debug.secretFileName(name) + ", size " + fileLength + ", torrent " + hash + ", access = "
                            + file.getAccessMode());
                }
            }
        } finally {
