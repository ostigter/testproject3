...
public class NonBlockingReadWriteService {
    ...
    public NonBlockingReadWriteService(String _service_name, int timeout, int close_delay, ServiceListener _listener) {
        ...
        if (timeout < TIMEOUT_CHECK_INTERVAL_MS / 1000)
            timeout = TIMEOUT_CHECK_INTERVAL_MS / 1000;
                ...
                try {
                    ...
                    if (messages != null) {
                        for (int i = 0; i < messages.length; i++) {
                            ...
                            ClientMessage client_msg = new ClientMessage(msg.getID(), client, msg.getMapPayload(), null); // note no handler. we let
                                                                                                                          // the listener attach it
                    ...
                    if (!client.isClosePending()) {
                        ...
                        // System.out.println( "[" +new Date()+ "] Connection read error [" +sc.socket().getInetAddress()+ "] ["
                        // +client.getDebugString()+ "]: " +t.getMessage() );
                    }
            ...
            public boolean selectSuccess(VirtualChannelSelector selector, SocketChannel sc, Object attachment) {
                try {
                    ...
                    if (more_writes_needed) {
                        write_selector.resumeSelects(client.getSocketChannel()); // we need to resume since write selects are auto-paused after select
                                                                                 // op
                    }

                    return (client.getLastWriteMadeProgress());
                } catch (Throwable t) {
                    // System.out.println( "[" +new Date()+ "] Connection write error [" +sc.socket().getInetAddress()+ "] ["
                    // +client.getDebugString()+ "]: " +t.getMessage() );
                    listener.connectionError(client, t);
                    return (false);
                }
                ...
                for (int i = 0; i < connections.size(); i++) {
                    ...
                    if (current_time < vconn.getLastActivityTime()) { // time went backwards!
                        ...
                        if (current_time - vconn.getLastActivityTime() > activity_timeout_period_ms
                                || (close_delay_period_ms > 0 && current_time - vconn.getLastActivityTime() > close_delay_period_ms)) {
