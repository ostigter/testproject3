...
public class TranscodeJobImpl implements TranscodeJob, DownloadWillBeRemovedListener {
    ...
    protected TranscodeJobImpl(TranscodeQueueImpl _queue, TranscodeTarget _target, TranscodeProfile _profile, DiskManagerFileInfo _file,
            boolean _add_stopped, int _transcode_requirement, boolean _is_stream)
    ...
    throws TranscodeException {
        transcode_file =
                ((DeviceImpl) target.getDevice()).allocateFile(profile, getTranscodeRequirement() == TranscodeTarget.TRANSCODE_NEVER, file, true);
    ...
    protected boolean canUseDirectInput() {
        ...
        return (file.getDownloaded() == length && file.getFile().length() == length);
    }

    protected boolean useDirectInput() {
        synchronized (this) {

            return (use_direct_input || (getPreferDirectInput() && canUseDirectInput()));
        }
    }
    ...
    public void generate(IndentWriter writer) {
        ...
        writer.println("stream=" + is_stream + ", state=" + state + ", treq=" + transcode_requirement + ", %=" + percent_complete + ", error="
                + error);
    }
}
