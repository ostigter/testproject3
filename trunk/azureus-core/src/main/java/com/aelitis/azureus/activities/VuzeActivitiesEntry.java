...
public class VuzeActivitiesEntry implements TableColumnSortObject {
    ...
    public void loadFromExternalMap(Map<?, ?> platformEntry) {
        ...
        setIconID(MapUtils.getMapString(platformEntry, "icon-url", MapUtils.getMapString(platformEntry, "icon-id", null)));
        setTypeID(MapUtils.getMapString(platformEntry, "type-id", null), true);
        setAssetHash(MapUtils.getMapString(platformEntry, "related-asset-hash", null));
        setAssetImageURL(MapUtils.getMapString(platformEntry, "related-image-url", null));
    ...
    public void setIconID(String iconID) {
        if (iconID != null && iconID.indexOf("image.") < 0 && !iconID.startsWith("http")) {
    ...
    /**
     * Independant for {@link #getDownloadManger()}. This will be written to the map.
     * 
    ...
    /**
     * Not needed if you {@link #setDownloadManager(DownloadManager)}. This will be written the map.
     * 
     * @param torrent
     * 
     * @since 3.0.5.3
