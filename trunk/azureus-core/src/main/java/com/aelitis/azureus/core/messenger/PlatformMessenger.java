...
public class PlatformMessenger {
    ...
    static private Map<String, Map<PlatformMessage, PlatformMessengerListener>> mapQueues =
            new HashMap<String, Map<PlatformMessage, PlatformMessengerListener>>();

    private static final String QUEUE_NOAZID = "noazid.";

    private static final String QUEUE_NORMAL = "msg.";

    static private AEMonitor queue_mon = new AEMonitor("v3.PlatformMessenger.queue");
    ...
    public static void queueMessage(PlatformMessage message, PlatformMessengerListener listener, boolean addToBottom) {
        ...
        try {
            ...
            if (message != null) {
                ...
                debug("q " + queueID + "(" + mapQueue.size() + ") " + message.toShortString() + ": " + message + " @ "
                        + new Date(message.getFireBefore()) + "; in " + (message.getFireBefore() - SystemTime.getCurrentTime()) + "ms");
                    ...
                    if (timerEvent == null || timerEvent.hasRun() || fireBefore < timerEvent.getWhen()) {
                        ...
                        timerEvent = timerProcess.addEvent(fireBefore, new TimerEventPerformer() {
                            public void perform(TimerEvent event) {
                                ...
                                /*
                                 * Object[] keys = mapQueues.keySet().toArray(); for (int i = 0; i < keys.length; i++) { Map mapQueue =
                                 * mapQueues.get(keys[i]); while (mapQueue != null && mapQueue.size() > 0) { processQueue(mapQueue); } }
                                 */
                            }
                        });
    ...
    protected static void processQueue(String queueID, Map mapQueue) {
        ...
        String sURL_RPC = ContentNetworkUtils.getUrl(cn, ContentNetwork.SERVICE_RPC) + "&" + urlStem.toString();
    ...
    private static Object[] downloadURL(URL rpc_url, String postData)
