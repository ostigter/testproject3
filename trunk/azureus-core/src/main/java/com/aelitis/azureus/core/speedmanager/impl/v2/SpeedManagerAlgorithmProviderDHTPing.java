...
/**
 * This class implements an Vivaldi strategy for adjusting the upload limit. It will every 30 seconds calculate the distance to the center of the
 * Vivaldi structure and if it determines that it is going away from it will lower the limit. If it getting closer it will increase the upload limit.
 */
...
public class SpeedManagerAlgorithmProviderDHTPing implements SpeedManagerAlgorithmProvider, COConfigurationListener {
    ...
    public void calculate(SpeedManagerPingSource[] sources) {
        ...
        if (limitMonitor.isConfTestingLimits()) {
            ...
            if (limitMonitor.isConfLimitTestFinished()) {
                ...
                SMUpdate ramp = limitMonitor.rampTestingLimit(adapter.getCurrentUploadLimit(), adapter.getCurrentDownloadLimit());
                logNewLimits(ramp);
                setNewLimits(ramp);
            }
    ...
    private void endLimitTesting() {
        ...
        SMUpdate update = limitMonitor.endLimitTesting(downLimitGuess, upLimitGuess);
    ...
    /**
     * The longer were get the same signal the stronger it is. On upticks however we only increase the rates when if the upload or download is
     * saturated.
     * 
     * @return -
     */
     ...
     * 
     * @param c
     *            - number of upsignals recieved in a row
