...
public class CacheDiscovery {
    ...
    private static boolean canBan(final String ip) {
    ...
    public static CachePeer categorisePeer(byte[] peer_id, final InetAddress ip, final int port) {
        for (int i = 0; i < discoverers.length; i++) {
    ...
    public static class CachePeerImpl implements CachePeer {
        ...
        public boolean sameAs(CachePeer other) {
            return (getType() == other.getType() && getAddress().getHostAddress().equals(other.getAddress().getHostAddress()) && getPort() == other
                    .getPort());
        }
