...
/**
 * Layout manager that lays out components in a grid of equally sized rows and
 * columns, starting in the top row and the most left column, to the next column
 * to the right until the container's width is reached, then wrapping to the
 * next row in the most left column again, automatically adding a vertical
 * scrollbar when needed. <br />
 * <br />
 * 
 * If the container is resized, the <code>revalidate</code> method should be
 * called to update the wrapping according to the container's new width and
 * height.
 * 
