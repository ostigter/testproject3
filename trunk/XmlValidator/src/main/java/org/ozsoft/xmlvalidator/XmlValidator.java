package org.ozsoft.xmlvalidator;
...
/**
 * Utility class to validate XML documents against their schema's.
 * 
 * The schema's must reside somewhere in the schema root directory.
 * 
 * Implemented using the JAXP API.
 * 
...
public class XmlValidator {
    ...
    public void validate(File file) throws SAXException, IOException {
        ...
        if (namespace != null && namespace.length() > 0) {
            ...
            if (validator != null) {
                ...
                System.out.println(String.format("%s: No schema found for namespace: %s", file, namespace));
            }
        } else {
            System.out.println(String.format("WARNING: %s: WARNING: No namespace declaration found.", file));
        }
    }
     ...
     */
    public String getNamespaceFromXmlFile(File file) throws SAXException, IOException {
        String namespace = null;
	 ...
	 * 
	 * Throws a ParsingDoneException when the root element namespace has been
	 * read, to abort further (unnecessary) pasing.
	 */
    private class DocumentHandler extends DefaultHandler {
         ...
         * Returns the target namespace.
         * 
         * @return The target namespace, or null if not present.
         */
        public String getNamespace() {
            return namespace;
        }
	 ...
	 * collected.
	 * 
	 * This trick boosts performance when parsing large documents, especially
	 * when the interesting data is the early parts of the document.
	 */
    private class ParsingDoneException extends SAXException {

        /** Serial version UID. */
    	private static final long serialVersionUID = 3111938985655269505L;
