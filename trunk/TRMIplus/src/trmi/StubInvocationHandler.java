...
/**
 * TRMI+ Accepts invocations from the client and forwards them to the remote
 * {@link RemoteObjectWrapper}, where they are executed.
 * <p>
 * 
...
public class StubInvocationHandler implements InvocationHandler, Serializable {
     ...
     * 
     * @param wrapper
     *            The remote object's wrapper. Invocation requests are delegated
     *            to it.
     * 
     * @param recoveryStrategy
     *            Strategy for recovering from remote errors.
     */
    public StubInvocationHandler(RemoteObjectWrapper wrapper, RemoteExceptionRecoveryStrategy recoveryStrategy) {
        this.wrapper = wrapper;
        this.recoveryStrategy = recoveryStrategy;
    }

    /**
     * TRMI+ Sends the invocation request to the remote object wrapper, and
     * returns the response. If the remote invocation fails, calls the recovery
     * strategy and tries again.
     * 
     * @exception RuntimeException
     *                If the recovery strategy fails.
     */
    ...
    /**
     * TRMI+ Handles java.lang.Object methods by forwarding them to the wrapper.
     */
     ...
     **/
    private Object invokeRemoteMethod(Object proxy, Method method, Object[] params) throws Throwable, RuntimeException {
        ...
        while (true) {
            try {
                ...
                convertPrimitiveParamTypes(convertedTypes, primitiveTypes);
                ...
                Object response = wrapper.invokeRemote(method.getName(), convertedTypes, primitiveTypes, params);

                return response;
            } catch (RemoteObjectWrapperException e) {
                // This indicates a bug in this suite
                throw new RuntimeException("Internal trmi error while invoking " + method + ": " + e);
                ...
                wrapper = recoveryStrategy.recoverFromRemoteException(wrapper, e);
     ...
     * 
     * @param types
     *            The types to convert. The primitive types are converted
     *            in-place.
     * 
     * @param primitiveTypes
     *            An out parameter that records which parameters in
     *            <code>types</code> are primitives and which aren't.
     */
    private void convertPrimitiveParamTypes(Class<?>[] types, boolean[] primitiveTypes)
        ...
        for (int i = 0; i < types.length; i++) {
            if (types[i].isPrimitive()) {
                ...
                if (types[i].equals(Boolean.TYPE)) {
                    ...
                    throw new IllegalArgumentException("Unrecognized primitive parameter type: " + types[i]);
                }
            } else {
    ...
    /**
     * Replaces method parameters with TRMI stubs as needed. See
     * {@link trmi.Naming#getParameterStubIfNeeded(Object, Class)} for more
     * details.
     */
