...
public class PlatformManagerImpl implements PlatformManager, AEWin32AccessListener, AEDiagnosticsEvidenceGenerator {
    ...
    protected void applyPatches() {
        try {
            ...
            String current = access.readStringValue(AEWin32Access.HKEY_CLASSES_ROOT, NEW_MAIN_ASSOC + "\\DefaultIcon", "");
            ...
            if (current.indexOf(app_exe_name) != -1 && !current.equals(target)) {
                ...
                writeStringToHKCRandHKCU(NEW_MAIN_ASSOC + "\\DefaultIcon", "", target);
            }
        } catch (Throwable e) {
    ...
    throws PlatformManagerException {
        /*
         * File exe_loc = getAureusEXELocation(); long size = exe_loc.length(); boolean old_exe = size < 250000; return(
         * old_exe?0:1);
         */
    ...
    protected File getApplicationEXELocation() throws PlatformManagerException {
        if (az_exe == null) {
            ...
            try {
                ...
                // OSTI: Disabled to avoid error about missing .EXE file.
                // if (!az_exe.exists()) {
                //
                // String msg = app_exe_name + " not found in " + az_home +
                // " - can't check file associations. Please re-install " + app_name;
                //
                // az_exe = null;
                //
                // if (!az_exe_checked) {
    ...
    throws PlatformManagerException {
        ...
        if (shared_options.exists()) {
            ...
            try {
                ...
                throw (new PlatformManagerException(MessageText.getString("platform.jvmopt.accesserror",
    ...
    throws PlatformManagerException {
        ...
        if (exe != null && exe.exists()) {
            ...
            try {
                String value = access.readStringValue(AEWin32Access.HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\Run", app_name);
        ...
        if (exe != null && exe.exists()) {
            ...
            try {
                ...
                if (run) {
                    ...
                    access.writeStringValue(AEWin32Access.HKEY_CURRENT_USER, key, app_name, exe.getAbsolutePath());
                } else {
        ...
        synchronized (this) {
            ...
            if (prevent_it) {
                ...
                if (prevent_sleep_thread == null) {
                    ...
                    prevent_sleep_thread = new AEThread2("SleepPreventer") {
                        @Override
                        public void run() {
                            while (true) {
    ...
    throws PlatformManagerException {
        ...
        try {
            if (type == SD_SLEEP) {
                ...
                Runtime.getRuntime().exec(new String[] { windir + "\\system32\\rundll32.exe", "powrprof.dll,SetSuspendState Sleep" });
                ...
                if (vista_or_higher) {
                    ...
                    Runtime.getRuntime().exec(new String[] { windir + "system32\\rundll32.exe", "powrprof.dll,SetSuspendState Hibernate" });
                }
    ...
    throws PlatformManagerException {
        ...
        try {
            String test1 = access.readStringValue(AEWin32Access.HKEY_CLASSES_ROOT, name + "\\shell\\open\\command", "");
            ...
            String test2 = access.readStringValue(AEWin32Access.HKEY_CLASSES_ROOT, type, "");
            ...
            try {
                String always_open_with = access.readStringValue(AEWin32Access.HKEY_CURRENT_USER,
                        "Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\FileExts\\" + type, "Application");
            ...
            /*
             * try{ String mru_list = access.readStringValue( AEWin32Access.HKEY_CURRENT_USER,
             * "Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\FileExts\\.torrent\\OpenWithList", "MRUList" );
             * //System.out.println( "mru_list = " + mru_list ); if ( mru_list.length() > 0 ){ String mru =
             ...
             * mru_list.charAt(0) ); //System.out.println( "mru = " + mru ); return(
             * mru.equalsIgnoreCase(app_exe_name)); } }catch( Throwable e ){ // e.printStackTrace(); // failure means
             * things are OK }
             */
        ...
        try {
            ...
            if (!magnet_exe_managing) {
                ...
                if (force || getAdditionalFileTypeRegistrationDetails("Magnet", ".magnet") == RT_NONE) {
                    ...
                    try {
                        registerAdditionalFileType("Magnet", "URL:Magnet Protocol", ".magnet", "application/x-magnet", true);
    ...
    @Override
    public void registerAdditionalFileType(String name, // e.g. "Azureus"
    ...
    throws PlatformManagerException {
        ...
        try {
            ...
            writeStringToHKCRandHKCU("MIME\\Database\\Content Type\\" + content_type, "Extension", type);

            writeStringToHKCRandHKCU(name, "", description);

            writeStringToHKCRandHKCU(name + "\\shell", "", "open");

            writeStringToHKCRandHKCU(name + "\\DefaultIcon", "", "\"" + az_exe_string + "\"," + getIconIndex());

            writeStringToHKCRandHKCU(name + "\\shell\\open\\command", "", "\"" + az_exe_string + "\" \"%1\"");
    ...
    private void writeStringToHKCRandHKCU(String subkey, String name, String value) {
        ...
        try {
            access.writeStringValue(AEWin32Access.HKEY_CURRENT_USER, "Software\\Classes\\" + subkey, name, value);
        } catch (Throwable e) {
            Debug.out(e);
        }
    }
    ...
    @Override
    public void unregisterAdditionalFileType(String name, // e.g. "Azureus"
            String type) // e.g. ".torrent"
        ...
        try {
            try {
                ...
                access.deleteValue(AEWin32Access.HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\FileExts\\" + type,
                        "Application");
            ...
            try {
                access.deleteKey(AEWin32Access.HKEY_CLASSES_ROOT, type);
            ...
            try {
                access.deleteKey(AEWin32Access.HKEY_CLASSES_ROOT, name, true);
            ...
            try {
                access.deleteKey(AEWin32Access.HKEY_CURRENT_USER, "Software\\Classes\\" + type);
            ...
            try {
                access.deleteKey(AEWin32Access.HKEY_CURRENT_USER, "Software\\Classes\\" + name, true);
    ...
    throws PlatformManagerException {
        try {
            access.writeWordValue(AEWin32Access.HKEY_LOCAL_MACHINE, "System\\CurrentControlSet\\Services\\Tcpip\\Parameters",
                    "DisableUserTOSSetting", enabled ? 0 : 1);
    ...
    throws PlatformManagerException {
        try {
            ...
            /*
             * Runtime.getRuntime().exec( new String[] { "explorer.exe", file.isDirectory() ? "/e," : "/e,/select,",
             * "\"" + file_name + "\"" });
             */
        } catch (Throwable e) {
