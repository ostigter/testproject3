    ...
    public void checkForUpdate(final UpdateChecker checker) {
        try {
            ...
            if (sf_comp_version.length() == 0 || !Character.isDigit(sf_comp_version.charAt(0))) {
                ...
                if (Logger.isEnabled())
                    Logger.log(new LogEvent(LOGID, LogEvent.LT_WARNING, "PlatformManager:Win32 no valid version to check against ("
                            + sf_comp_version + ")"));
            ...
            if (target_version != null) {
                ...
                final Update update = checker.addUpdate(UPDATE_NAME, update_d, target_version, alternate_rdl, Update.RESTART_REQUIRED_YES);
