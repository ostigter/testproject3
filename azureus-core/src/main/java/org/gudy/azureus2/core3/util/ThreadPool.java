...
public class ThreadPool {
    private static final boolean NAME_THREADS = Constants.IS_CVS_VERSION
            && System.getProperty("az.thread.pool.naming.enable", "true").equals("true");
    ...
    private void generateEvidence(IndentWriter writer) {
        writer.println(name + ": max=" + max_size + ",qwf=" + queue_when_full + ",queue=" + task_queue.size() + ",busy=" + busy.size() + ",total="
                + task_total + ":" + DisplayFormatters.formatDecimal(task_average.getDoubleAverage(), 2) + "/sec");
    }
        ...
        if (log_cpu) {
            ...
            if (start_cpu > 0) {
                ...
                if (diff_cpu > 10 || diff_millis > 10) {
                    ...
                    System.out.println(TimeFormatter.milliStamp() + ": Thread: " + Thread.currentThread().getName() + ": " + runnable + " -> "
                            + diff_cpu + "/" + diff_millis);
                }
            }
        } else {
    ...
    protected void checkTimeouts() {
        synchronized (this) {
            ...
            if (debug_thread_pool_log_on) {
                ...
                System.out.println("ThreadPool '" + getName() + "'/" + thread_name_index + ": max=" + max_size + ",sem=[" + thread_sem.getString()
                        + "],busy=" + busy.size() + ",queue=" + task_queue.size());
            }
                                ...
                                synchronized (busy_pools) {
                                    if (!busy_pools.contains(ThreadPool.this)) {
                                        ...
                                        if (!busy_pool_timer_set) {
                                            ...
                                            COConfigurationManager.addAndFireParameterListeners(new String[] { "debug.threadpool.log.enable",
                                                    "debug.threadpool.debug.trace" }, new ParameterListener() {
                                                public void parameterChanged(String name) {
                                                    debug_thread_pool =
                                                            COConfigurationManager.getBooleanParameter("debug.threadpool.log.enable", false);
                                                    debug_thread_pool_log_on =
                                                            COConfigurationManager.getBooleanParameter("debug.threadpool.debug.trace", false);
                                                }
                                            });
