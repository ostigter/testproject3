...
public class Main {
    ...
    public static String DEFAULT_UI = "console";
    ...
    private static CommandLine parseCommands(String[] args, boolean constart) {
        ...
        if (commands != null && commands.hasOption('h')) {
            if (constart) {
                ...
                hf.printHelp(
                        "java org.gudy.azureus2.ui.common.Main",
                        "Optionally you can put torrent files to add to the end of the command line.\r\n",
                        options,
                        "Available User Interfaces: swt (default), web, console\r\nThe default interface is not started if you give either the '-e' or '-c' option (But you can start it by hand with '-u').",
                        true);
                System.exit(0);
            }
        }
    ...
    public static class StartSocket {
        public StartSocket(String args[]) {
            ...
            try {
                ...
                // NOTE - this formatting is also used by AzureusCoreSingleInstanceClient and other
