...
final class SocketServer implements Runnable {
    ...
    /**
     * start up the server socket and when a new connection is received, check that the source address is in our permitted list and if so, start a new
     * console input on that socket.
     */
    public void run() {
        int threadNum = 1;
                ...
                if (addr.isUnresolved() || !isAllowed(addr)) {
                    ...
                    while (true) {
                        ...
                        if (profile != null) {
                            ...
                            ui.createNewConsoleInput("Telnet Console " + threadNum++, socket.getInputStream(), new PrintStream(socket
                                    .getOutputStream()), profile);
                            break;
                        }
                        ...
                        if (loginAttempts >= maxLoginAttempts) {
                            System.out.println("TelnetUI: rejecting connection from: " + addr
                                    + " as number of failed connections > max login attempts (" + maxLoginAttempts + ")");
                            socket.close();
                            break;
                        }
                    }
    ...
    /**
     * if usermanager is null (ie: multi user is not enabled), returns the default user profile otherwise, requests username and password and
     * authenticates user before returning the user profile for this user
     * 
     * @param in
