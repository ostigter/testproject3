...
/**
 * Is the application in a "download" mode? Or is it in a "seeding" mode? This is used to determine up we cut back on upload bandwidth limit.
 * 
 * Here is how to determine the mode. If the download rate is LOW compared to the capacity for five minutes continously then it will be considered in
 * a SEEDING mode.
 * 
 * If the download bandwidth ever goes into the MED range then it switches to DOWNLOADING mode immediately.
 * 
...
public class TransferMode {
    ...
    /**
     * If the download bandwidth is ever in MED or above switch immediately to DOWNLOADING mode. If th download bandwidth is LOW or less for more then
     * 5 min, switch to SEEDING mode.
     * 
    ...
    /**
     * We have two types of limit testing. If we are doing a "confidence test" for the limits then return true. This mode is putting one value at the
     * min setting and the other at unlimited.
     * 
