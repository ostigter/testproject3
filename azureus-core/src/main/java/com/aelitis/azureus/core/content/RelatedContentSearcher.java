...
public class RelatedContentSearcher implements DistributedDatabaseTransferHandler {
                    ...
                    try {
                        ...
                        try {
                            ...
                            final Map<InetSocketAddress, DistributedDatabaseContact> contact_map =
                                    new HashMap<InetSocketAddress, DistributedDatabaseContact>();
                                        ...
                                        if (!contact_map.containsKey(address)) {
                                            ...
                                            try {
                                                DistributedDatabaseContact c =
                                                        ddb.importContact(address, DHTTransportUDP.PROTOCOL_VERSION_MIN, network == DHT.NW_CVS
                                                                ? DistributedDatabase.DHT_CVS : DistributedDatabase.DHT_MAIN);
                                    ...
                                    public void run() {
                                        try {
                                            ...
                                            List<DistributedDatabaseContact> extra_contacts =
                                                    sendRemoteSearch(si, hashes_sync_me, contact_to_search, term, observer);
                                            ...
                                            if (extra_contacts == null) {
                                                ...
                                                logSearch("    " + contact_to_search.getAddress() + " OK " + type + " - additional="
                                                        + extra_contacts.size());
    ...
    protected List<DistributedDatabaseContact> sendRemoteSearch(SearchInstance si, Set<String> hashes_sync_me, DistributedDatabaseContact contact,
            String term, SearchObserver observer) {
        try {
            ...
            }, manager.transfer_type, key, 10000);
                ...
                SearchResult result = new SearchResult() {
                    public Object getProperty(int property_name) {
                        try {
                            if (property_name == SearchResult.PR_NAME) {
                            ...
                            } else if (property_name == SearchResult.PR_DOWNLOAD_LINK || property_name == SearchResult.PR_DOWNLOAD_BUTTON_LINK) {

                                byte[] hash = (byte[]) map.get("h");

                                if (hash != null) {

                                    return (UrlUtils.getMagnetURI(hash, title, RelatedContentManager.convertNetworks((byte) ImportExportUtils
                                            .importLong(map, "o", RelatedContentManager.NET_PUBLIC))));
                                }
                            } else if (property_name == RelatedContentManager.RCM_SEARCH_PROPERTY_CONTENT_NETWORK) {
                                ...
                                return (RelatedContentManager.convertNetworks((byte) ImportExportUtils.importLong(map, "o",
                                        RelatedContentManager.NET_PUBLIC)));
                            }
                ...
                for (Map<String, Object> m : list) {
                    ...
                    try {
                        ...
                        DistributedDatabaseContact ddb_contact =
                                ddb.importContact(new InetSocketAddress(InetAddress.getByName(host), port), DHTTransportUDP.PROTOCOL_VERSION_MIN,
                                        contact.getDHT());
    ...
    protected BloomFilter sendRemoteFetch(DistributedDatabaseContact contact) {
        try {
            ...
            }, manager.transfer_type, key, 5000);
    ...
    protected BloomFilter sendRemoteUpdate(ForeignBloom f_bloom) {
        try {
            ...
            DistributedDatabaseValue value = f_bloom.getContact().read(new DistributedDatabaseProgressListener() {
                public void reportSize(long size) {
                }
    ...
    protected Map<String, Object> receiveRemoteRequest(DistributedDatabaseContact originator, Map<String, Object> request) {
        ...
        try {
            ...
            if (req_type != null) {
                ...
                logSearch("Received remote request: " + BDecoder.decodeStrings(request) + " from " + originator.getAddress() + "/"
                        + originator.getDHT() + ", dup=" + dup + ", bs=" + harvest_op_requester_bloom.getEntryCount());
                ...
                logSearch("Received remote search: '" + term + "' from " + originator.getAddress() + ", hits=" + hits + ", bs="
                        + harvest_se_requester_bloom.getEntryCount());
    ...
    public DistributedDatabaseValue write(DistributedDatabaseContact contact, DistributedDatabaseTransferType type, DistributedDatabaseKey key,
    ...
    private List<String> getDHTWords(DownloadInfo info) {
    ...
    protected void updateKeyBloom(RelatedContentManager.ContentCache cc) {
        synchronized (manager.rcm_lock) {
            ...
            logSearch("blooms=" + all_bloom.getSize() + "/" + all_bloom.getEntryCount() + ", " + non_dht_bloom.getSize() + "/"
                    + non_dht_bloom.getEntryCount() + ": rcm=" + cc.related_content.size() + ", trans="
                    + RelatedContentManager.transient_info_cache.size() + ", dht=" + dht_infos.size());
    ...
    private List<DownloadInfo> getDHTInfos() {
    ...
    private void testKeyBloom() {
        ...
        try {
            ...
            System.out.println("total=" + total + ", clash=" + clashes + ", miss=" + misses + ", fails=" + match_fails + ", bloom="
                    + bloom.getString());
                    ...
                    try {
                        ...
                        outer: for (DHT dht : dhts) {
                            ...
                            for (DHTTransportContact contact : contacts) {
                                ...
                                DistributedDatabaseContact ddb_contact =
                                        ddb.importContact(contact.getAddress(), DHTTransportUDP.PROTOCOL_VERSION_MIN, network == DHT.NW_CVS
                                                ? DistributedDatabase.DHT_CVS : DistributedDatabase.DHT_MAIN);
    ...
    private static class MySearchObserver implements SearchObserver {
