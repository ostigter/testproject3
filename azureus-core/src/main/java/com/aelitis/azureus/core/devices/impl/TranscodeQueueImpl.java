...
public class TranscodeQueueImpl implements TranscodeQueue {
        ...
        try {
            ...
            if (xcode_required) {
                                ...
                                if (error[0] == null) {
                                    ...
                                    if (contiguous_downloaded < file_size) {
                                        ...
                                        if (current_downloaded < file_size) {
                                            ...
                                            Debug.out("Premature transcode termination: init=" + initial_file_downloaded + ", curr="
                                                    + current_downloaded + ", len=" + file_size);
                ...
                if (job.isStream()) {
                    ...
                    /*
                     * provider_job[0] = provider.transcode( adapter, job.getFile(), profile, new File( "C:\\temp\\arse").toURI().toURL());
                     */
                    ...
                    provider_job[0] =
                            provider.transcode(xcode_adapter, provider_analysis, direct_input, job.getFile(), profile, new URL("tcp://127.0.0.1:"
                                    + pipe.getPort()));
                    ...
                    provider_job[0] =
                            provider.transcode(xcode_adapter, provider_analysis, direct_input, job.getFile(), profile, output_file.toURI().toURL());
                }
    ...
    public TranscodeJobImpl add(TranscodeTarget target, TranscodeProfile profile, DiskManagerFileInfo file, boolean add_stopped)

    throws TranscodeException {
        return (add(target, profile, file, add_stopped, false, TranscodeTarget.TRANSCODE_UNKNOWN));
    }

    public TranscodeJobImpl add(TranscodeTarget target, TranscodeProfile profile, DiskManagerFileInfo file, int transcode_requirement,
            boolean add_stopped)
    ...
    public TranscodeJobImpl add(TranscodeTarget target, TranscodeProfile profile, DiskManagerFileInfo file, boolean add_stopped, boolean stream,
