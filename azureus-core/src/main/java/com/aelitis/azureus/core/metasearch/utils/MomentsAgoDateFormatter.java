...
public class MomentsAgoDateFormatter {
    ...
    /**
     * Returns "x <units of time> ago on <formatted pastDate>" by comparing the given pastDate with the current time. All formats are converted to GMT
     * time. In the future the user might have their own locale, in which case we will display the time in their own locale (neat!)
     * 
     * @param pastDate
     *            A date in the past
     * @param format
     *            The format for the pastDate
    ...
    /**
     * Returns "x <units of time> ago on <formatted pastDate>" by comparing the given pastDate with the current time.
     * 
     * @param pastDate
     *            A default locale date in the past
     ...
     * 
     * @param then
     *            The date we're evaluating
     * @param now
     *            The current time
     * @param field
     *            The field which we're evaluating ("units")
     * @return null if then is 0 "units" from now, otherwise a displayable string that will notify the user how long ago then was from now.
     */
