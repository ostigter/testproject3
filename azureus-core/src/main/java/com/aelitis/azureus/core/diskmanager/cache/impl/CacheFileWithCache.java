...
public class CacheFileWithCache implements CacheFile {
    ...
    protected void updateStats() {
        ...
        // System.out.println( "read-ahead: done = " + read_ahead_bytes_made + ", used = " + read_ahead_bytes_used + ", done_av = " +
        // read_ahead_made_average.getAverage() + ", used_av = " + read_ahead_used_average.getAverage()+ ", size = " + current_read_ahead_size );
    }

    protected void
            readCache(final DirectByteBuffer file_buffer, final long file_position, final boolean recursive, final boolean disable_read_cache)
        ...
        try {
            if (manager.isCacheEnabled()) {
                ...
                if (TRACE)
                    Logger.log(new LogEvent(torrent, LOGID, "readCache: " + getName() + ", " + file_position + " - "
                            + (file_position + read_length - 1) + ":" + file_buffer_position + "/" + file_buffer_limit));
                    ...
                    while (ok && writing_left > 0 && it.hasNext()) {
                        ...
                        if (entry_file_position > writing_file_position) {
                            ...
                            try {
                                ...
                                if (TRACE)
                                    Logger.log(new LogEvent(torrent, LOGID, "cacheRead: using " + entry.getString() + "["
                                            + entry_buffer.position(SS_CACHE) + "/" + entry_buffer.limit(SS_CACHE) + "]" + "to write to ["
                                            + file_buffer.position(SS_CACHE) + "/" + file_buffer.limit(SS_CACHE) + "]"));
                    ...
                    for (int i = 0; i < 2; i++) {
                        ...
                        try {
                            boolean do_read_ahead =
                                    i == 0
                                            && // first time round
                                            !recursive && !disable_read_cache && read_history != null && manager.isReadCacheEnabled()
                                            && read_length < current_read_ahead_size && file_position + current_read_ahead_size <= file.getLength();
                            ...
                            if (do_read_ahead) {
                                ...
                                try {
                                    ...
                                    CacheEntry entry =
                                            manager.allocateCacheSpace(CacheEntry.CT_READ_AHEAD, this, cache_buffer, file_position,
                                                    actual_read_ahead);
            ...
            if (AEDiagnostics.CHECK_DUMMY_FILE_DATA) {
                ...
                while (file_buffer.hasRemaining(SS_CACHE)) {
                    ...
                    if ((byte) temp_position != v) {
                        ...
                        System.out.println("readCache: read is bad at " + temp_position + ": expected = " + (byte) temp_position + ", actual = "
                                + v);
    ...
    throws CacheFileManagerException {
        ...
        try {
            ...
            if (AEDiagnostics.CHECK_DUMMY_FILE_DATA) {
                ...
                while (file_buffer.hasRemaining(SS_CACHE)) {
                    ...
                    if ((byte) temp_position != v) {
                        ...
                        System.out.println("writeCache: write is bad at " + temp_position + ": expected = " + (byte) temp_position + ", actual = "
                                + v);
            ...
            if (manager.isWriteCacheEnabled()) {
                ...
                if (TRACE)
                    Logger.log(new LogEvent(torrent, LOGID, "writeCache: " + getName() + ", " + file_position + " - "
                            + (file_position + write_length - 1) + ":" + file_buffer_position + "/" + file_buffer_limit));
    ...
    protected void flushCache(long file_position, long length, // -1 -> do all from position onwards
            boolean release_entries, long minimum_to_release, // -1 -> all
    ...
    protected void flushCacheSupport(long file_position, long length, // -1 -> do all from position onwards
            boolean release_entries, long minimum_to_release, // -1 -> all
            long oldest_dirty_time, // dirty entries newer than this won't be flushed
                                    // 0 -> now
            long min_chunk_size) // minimum contiguous size for flushing, -1 -> no limit
                ...
                try {
                    ...
                    if (dirty && (oldest_dirty_time == 0 || entry.getLastUsed() < oldest_dirty_time)) {
                        ...
                        if (multi_block_start == -1) {
                            ...
                            if (skip_chunk) {
                                if (TRACE)
                                    Logger.log(new LogEvent(torrent, LOGID, "flushCache: skipping " + multi_block_entries.size() + " entries, ["
                                            + multi_block_start + "," + multi_block_next + "] as too small"));
                            } else {
    ...
    throws CacheFileManagerException {
        ...
        try {
            if (TRACE)
                Logger.log(new LogEvent(torrent, LOGID, "multiBlockFlush: writing " + multi_block_entries.size() + " entries, ["
                        + multi_block_start + "," + multi_block_next + "," + release_entries + "]"));
            ...
            if (expected_per_entry_write != expected_overall_write) {
                ...
                throw (new CacheFileManagerException(this, "flush: inconsistent write length, entrys = " + expected_per_entry_write + " overall = "
    ...
    throws CacheFileManagerException {
        if (manager.isCacheEnabled()) {
            ...
            if (TRACE)
                Logger.log(new LogEvent(torrent, LOGID, "flushCache: " + getName() + ", rel = " + release_entries + ", min = " + minumum_to_release));
    ...
    throws CacheFileManagerException {
        if (manager.isCacheEnabled()) {
            ...
            if (TRACE)
                Logger.log(new LogEvent(torrent, LOGID, "flushOldDirtyData: " + getName()));
    ...
    protected void getBytesInCache(boolean[] toModify, long[] absoluteOffsets, long[] lengths) {
        ...
        try {
            ...
            while (it.hasNext()) {
                ...
                if (startPos >= absoluteOffsets[i] && endPos >= absoluteOffsets[i] + lengths[i]) { // chunk completely falls into cache entry -> don't
                                                                                                   // invalidate
                    i++;
                    doSkipping = true;
