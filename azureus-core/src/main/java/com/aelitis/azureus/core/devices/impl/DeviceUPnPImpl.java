...
public abstract class DeviceUPnPImpl extends DeviceImpl implements TranscodeTargetListener, DownloadManagerListener {
    ...
    public void associate(UnassociatedDevice assoc) {
        if (isAlive()) {
    ...
    protected void addDynamicXCode(final DiskManagerFileInfo source) {
        ...
        try {
            ...
            final DiskManagerFileInfo stream_file = new DiskManagerFileInfoStream(new DiskManagerFileInfoStream.StreamFactory() {
                private List<Object> current_requests = new ArrayList<Object>();

                public StreamDetails getStream(Object request)

                throws IOException {
                    try {
                        TranscodeJobImpl job =
                                getManager().getTranscodeManager().getQueue().add((TranscodeTarget) DeviceUPnPImpl.this, profile, source, false,
                                        true, TranscodeTarget.TRANSCODE_UNKNOWN);
    ...
    protected void removeDynamicXCode(final DiskManagerFileInfo source) {
        final TranscodeProfile profile = dynamic_transcode_profile;
    ...
    protected void syncCategoriesAndTags(TranscodeFileImpl tf, boolean inherit_from_download) {
        try {
            ...
            if (dl != null) {
                ...
                TagManagerFactory.getTagManager().getTagType(TagType.TT_DOWNLOAD_MANUAL).addTagListener(PluginCoreUtils.unwrap(dl),
                        new TagListener() {
    ...
    protected static class StreamWrapper implements DiskManagerFileInfoStream.StreamFactory.StreamDetails {
        ...
        public boolean hasFailed() {
            ...
            while (true) {
                ...
                if (state == TranscodeJobImpl.ST_FAILED || state == TranscodeJobImpl.ST_CANCELLED || state == TranscodeJobImpl.ST_REMOVED
