...
public class DeviceDriveManager implements DriveDetectedListener {
    ...
    protected DeviceDriveManager(DeviceManagerImpl _manager) {
        ...
        manager.addListener(new DeviceManagerListener() {
        ...
        async_dispatcher.dispatch(new AERunnable() {
            public void runSupport() {
                ...
                if ((sVendor.equalsIgnoreCase("htc") && sProdID.equalsIgnoreCase("android phone"))
                        || (sVendor.toLowerCase().contains("motorola") && sProdID.length() > 0) || sVendor.equalsIgnoreCase("samsung")) {
    ...
    protected DeviceMediaRendererManual addDevice(String target_name, String target_classification, File root, File target_directory,
            boolean generic) {
        ...
        if (renderer == null) {
            ...
            try {
                renderer =
                        (DeviceMediaRendererManual) manager.createDevice(Device.DT_MEDIA_RENDERER, null, target_classification, target_name, true);
