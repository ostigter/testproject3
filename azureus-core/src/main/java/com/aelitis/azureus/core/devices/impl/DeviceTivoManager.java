...
public class DeviceTivoManager {
    ...
    throws IOException {
        String beacon =
                "tivoconnect=1" + LF + "swversion=1" + LF + "method=" + (is_broadcast ? "broadcast" : "connected") + LF + "identity=" + uid + LF
                        + "machine=" + device_manager.getLocalServiceName() + LF + "platform=pc" + LF + "services=TiVoMediaServer:" + my_port
                        + "/http";
    ...
    protected class Searcher {
        ...
        throws DeviceManagerException {
            try {
                ...
                twc.addPageGenerator(new TrackerWebPageGenerator() {
                ...
                timer_event = SimpleTimer.addPeriodicEvent("Tivo:Beacon", 60 * 1000, new TimerEventPerformer() {
                ...
                if (_async) {
                    ...
                    new DelayedEvent("search:delay", 5000, new AERunnable() {
