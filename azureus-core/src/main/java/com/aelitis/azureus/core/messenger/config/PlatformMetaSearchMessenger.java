...
public class PlatformMetaSearchMessenger {
    ...
    protected static templateInfo getTemplateInfo(Map m) {
        ...
        if (id == null || show == null || date == null) {
            ...
            /*
             * SimpleDateFormat format = new SimpleDateFormat( "yyyy-MM-dd HH:mm:ss.S"); format.setTimeZone( TimeZone.getTimeZone( "UTC" )); try{ long
             * millis = format.parse( date ).getTime(); return( new templateInfo( id.longValue(), millis, show.booleanValue())); }catch( Throwable e
