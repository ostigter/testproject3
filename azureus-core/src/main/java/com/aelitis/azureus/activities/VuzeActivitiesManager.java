...
public class VuzeActivitiesManager {
    ...
    private static void setupContentNetwork(final ContentNetwork cn) {
        cn.addPersistentPropertyChangeListener(new ContentNetworkPropertyChangeListener() {
            ...
            public void propertyChanged(String name) {
                ...
                boolean isActive = (oIsActive instanceof Boolean) ? ((Boolean) oIsActive).booleanValue() : false;
                if (isActive) {
        ...
        /*
         * final String id_str = cn.getServiceURL( ContentNetwork.SERVICE_IDENTIFY ); if ( id_str != null && id_str.length() > 0 ){ try{
         * SimpleTimer.addPeriodicEvent( "act:id", 23*60*60*1000, new TimerEventPerformer() { public void perform( TimerEvent event ) { identify( cn,
         * id_str ); } }); identify( cn, id_str ); }catch( Throwable e ){ Debug.out( e ); } }
         */
    }
     ...
     * 
     * @param delay
     *            max time to wait before running request
     * 
     * @since 3.0.4.3
     */
    public static void pullActivitiesNow(long delay, String reason, boolean alwaysPull) {
        /*
         * ContentNetworkManager cnm = ContentNetworkManagerFactory.getSingleton(); if (cnm == null) { return; } ContentNetwork[] contentNetworks =
         * cnm.getContentNetworks(); for (ContentNetwork cn : contentNetworks) {
         */
        {
    ...
    private static void loadEvents() {
        ...
        try {
            ...
            if (map != null && map.size() > 0 && MapUtils.getMapLong(map, "version", 0) < 2) {
                clearLastPullTimes();
                skipAutoSave = false;
                saveEventsNow();
                return;
            ...
            if (lastNewsAt.size() == 0) {
                ...
                if (lastVuzeNewsAt > 0) {
                    ...
                    lastNewsAt.put("" + ContentNetwork.CONTENT_NETWORK_VUZE, new Long(lastVuzeNewsAt));
                }
            }

            Object value;

            List newRemovedEntries = (List) MapUtils.getMapObject(map, "removed-entries", null, List.class);
